...
export class HeroService {
  ...
  updateHero(hero: Hero): Observable<any> {
    return this.http.put<Hero>(this.heroesUrl, hero, this.httpOptions).pipe(tap(_ => this.log(`Updated Hero ${hero.id}`)), catchError(this.handleError<any>(`updateHero id=${hero.id}`)));
  }

  addHero(hero: Hero): Observable<Hero> {
    return this.http.post<Hero>(this.heroesUrl, hero, this.httpOptions).pipe(tap((newHero: Hero) => this.log(`Added Hero ${hero.name}`)), catchError(this.handleError<Hero>(`addHero id=${hero.name}`)));
  }
